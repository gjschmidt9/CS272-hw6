...
function addToCart() {
    const amn = getAmount();
    const errorText = document.getElementById("amn-error-text");

    alert("I should add to the cart!");

    if (!amn || amn <= 0) {
        errorText.innerText = "Please enter a positive number for an amount.";
        return;
    }
...
function checkout() { 
    ...
    const isEmpty = (!cart.apple && !cart.banana && !cart.coconut);
    if (isEmpty) {
        errorText.innerText = "You must have something in your cart to checkout!";
        return;
    }
...
function resetCart() {
    ...
    document.getElementById("amn-error-text").textContent = "";
